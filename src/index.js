			...
			schema: {
				...
				properties: {
					key_learnings: {
						type: 'array',
						description:
							"An array containing only the user's responses that answer the question 'Any key learnings or next actions?'. The original question should not be included in the output.",
						items: { type: 'string' },
					},
				},
			...
			schema: {
				...
				properties: {
					blocked: {
						type: 'array',
						description:
							"An array containing only the user's responses that answer the question 'What blocked you from doing this action?'. The original question should not be included in the output.",
						items: { type: 'string' },
					},
					do_next: {
						type: 'array',
						description:
							"An array containing only the user's responses that answer the question 'What will you do to ensure you don't get blocked again?'. The original question should not be included in the output.",
						items: { type: 'string' },
					},
				},
				required: ['blocked', 'do_next'],
				additionalProperties: false,
			},
		},
	},
			...
			schema: {
				...
				properties: {
					why_not: {
						type: 'array',
						description:
							"An array containing only the user's responses that answer the question 'Why not?'. The original question should not be included in the output.",
						items: { type: 'string' },
					},
				},
				required: ['why_not'],
	...
	for (const key in data) {
		if (Array.isArray(data[key])) {
			...
			if (match) {
				...
				const renderedItems = data[key].map((item) => itemTemplate.replace(/{{this}}/g, item)).join('');
	...
	async fetch(request, env, ctx) {
		try {
			...
			if (!body.status || !validStatuses.includes(body.status)) {
				...
				return new Response(JSON.stringify({ error: 'Invalid request: status must be one of done, not_done, or canceled' }), {
					status: 400,
					headers: {
						'Content-Type': 'application/json',
					},
			...
			if (!body.text || typeof body.text !== 'string' || body.text.trim() === '') {
				...
				return new Response(JSON.stringify({ error: 'Invalid request: text must be a non-empty string' }), {
					status: 400,
					headers: {
						'Content-Type': 'application/json',
					},
			...
			try {
				...
				const aiResponse = await callOpenAI(systemMessage, body.text, jsonSchema, env.CF_TOKEN, env.OPENAI_API_KEY);
